...
class JournalController extends Controller
{
    ...
    public function journalsByUser($userId)
    {
        ...
        $vents = Vente::where('user_id', $userId)->orderBy('created_at', 'desc')->get();;
        $ventes = $vents->groupBy(function($m) {
            return $m->created_at->format('d-m-Y');
        });

        $deps = Depense::where('user_id', $userId)->orderBy('created_at', 'desc')->get();
        $allDepenses = $deps->groupBy(function($m) {
            return $m->created_at->format('d-m-Y');
        });
